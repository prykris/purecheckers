  ...
  <section class="invitations card" aria-label="Game invitations">
    ...
    {#each invitations as invite (invite.roomId)}
      <div class="invitation">
        <p><strong><PlayerLink username={invite.hostName} /></strong> invited you to play. No buy-in · {invite.turnTimer}s turns.</p>
        ...
        <button class="btn btn-dark btn-small" disabled={!canSend}
          on:click={() => sendCommand('challenge:decline', { roomId: invite.roomId })}>Decline</button>
      </div>
    {/each}
