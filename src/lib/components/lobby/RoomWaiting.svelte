              ...
              <div class="player-row empty">
                ...
                {#if isHost}
                  <div class="bot-inline">
                    <label class="bot-chip easy" class:selected={botDifficulty === 'easy'}>
                      <input type="radio" name="botdiff" value="easy" bind:group={botDifficulty} on:change={() => setBotDifficulty('easy')} />
                    </label>
                    <label class="bot-chip medium" class:selected={botDifficulty === 'medium'}>
                      <input type="radio" name="botdiff" value="medium" bind:group={botDifficulty} on:change={() => setBotDifficulty('medium')} />
                    </label>
                    <label class="bot-chip hard" class:selected={botDifficulty === 'hard'}>
                      <input type="radio" name="botdiff" value="hard" bind:group={botDifficulty} on:change={() => setBotDifficulty('hard')} />
                    </label>
                    <button class="btn btn-dark btn-small" on:click={callBot}>Bot</button>
                  </div>
                {/if}
              </div>
            {/if}
...
<style>
  ...
  .bot-inline { display: flex; align-items: center; gap: var(--sp-xs); margin-left: auto; }
  .bot-chip {
    width: 14px; height: 14px; border-radius: 50%; cursor: pointer;
    border: 2px solid; display: flex; align-items: center; justify-content: center;
    transition: background 0.15s;
  }
  .bot-chip input { display: none; }
  .bot-chip.easy { border-color: var(--success); }
  .bot-chip.medium { border-color: var(--warning); }
  .bot-chip.hard { border-color: var(--accent); }
  .bot-chip.easy.selected { background: var(--success); }
  .bot-chip.medium.selected { background: var(--warning); }
  .bot-chip.hard.selected { background: var(--accent); }
